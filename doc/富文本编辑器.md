...
[和其他富文本编辑器比较官网链接](https://quilljs.com/guides/comparison-with-other-rich-text-editors/)

和CKEditor与TinyMCE比较，Quill采用了不同的实现方式。

CKEditor与TinyMCE操作的数据和展现给用户的视图层是同一份 HTML／DOM，HTML 是树状结构，显然树状结构不如线性结构好处理，而 Quill 内部就是通过使用线性结构的方式使操作富文本编辑器变得简单，而且数据层和视图层是分离的，这让 Quill 对现在很流行的 React、Vue 或者 Angular 都能很好的支持。

如果项目需要支持老版本的浏览器，或者需要直接操作DOM元素的innerHTML。则建议采用CKEditor与TinyMCE
